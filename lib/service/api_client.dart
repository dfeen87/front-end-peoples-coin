...
class PeoplesCoinApiClient {
  ...
  PeoplesCoinApiClient({String? baseUrl})
      : _baseUrl = baseUrl ?? "https://peoples-coin-service-105378934751.us-east4.run.app";
  ...
  Future<List<GoodwillAction>> getUserGoodwillActions(String userId) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to fetch goodwill actions');
    }
  }
  ...
  Future<Map<String, dynamic>> submitGoodwill(GoodwillActionToSend actionToSend) async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      throw Exception('Failed to submit goodwill');
    }
  }
  ...
  Future<Proposal> getProposalDetails(String proposalId) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to fetch proposal details');
    }
  }
  ...
  Future<List<PublicLedgerEntry>> getPublicLedger() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to fetch public ledger');
    }
  }
  ...
  Future<String> checkMetabolicStatus() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to check metabolic status');
    }
  }
}
