...
class GoodwillProcessingProvider with ChangeNotifier {
  ...
  Future<String> _getIdToken() async {
    ...
    if (token == null || token.isEmpty) throw Exception('Failed to get Firebase ID token.');
    return token;
  }
  ...
  Future<bool> submitGoodwill({
    ...
    // Create a pending GoodwillAction object to track
    final pendingAction = GoodwillAction(
      id: null, // not yet assigned by backend
      performerUserId: performerUserId,
      actionType: actionType,
      description: description,
      lovesValue: lovesValue,
      ...
      status: 'pending', // you may want to add a status field in GoodwillAction model
    );
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        _pendingSubmissions.removeWhere((action) =>
            action.timestamp == pendingAction.timestamp &&
